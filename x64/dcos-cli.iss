...
Source: "python-3.4.3.amd64.msi"; DestDir: "{tmp}"
Source: "python-2.7.10.amd64.msi"; DestDir: "{tmp}"
Source: "Git-2.5.1-64-bit.exe"; DestDir: "{tmp}"
Source: "dcoscli-0.1.13-py2.py3-none-any.whl"; DestDir: "{tmp}"
Source: "virtualenv-13.1.2-py2.py3-none-any.whl"; DestDir: "{tmp}"
Source: "pywin32-219.win-amd64-py2.7.exe"; DestDir: "{tmp}"
Source: "pywin32-219.win-amd64-py3.4.exe"; DestDir: "{tmp}"
...
begin
  ...
  test3 := RegKeyExists(HKLM64, 'SOFTWARE\Python\PythonCore\2.7\InstallPath');
  test4 := RegKeyExists(HKLM64, 'SOFTWARE\Python\PythonCore\3.4\InstallPath');
...
var
  test: boolean;
begin
  test :=  RegKeyExists(HKLM64, 'SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\Git_is1');
  result := test
end;
...
begin
  ...
  Page2 := CreateInputQueryPage(wpInfoAfter, 'DCOS verification', '', 'Please go to that url and enter verification code. You can skip this step');
  Page2.Add('url: ', False)
  ...
  CheckListBox.AddCheckBox('Python', '', 0, Ptest, not Ptest, False, False, nil);
